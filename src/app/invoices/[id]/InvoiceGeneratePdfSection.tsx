...
export default function InvoiceGeneratePdfSection({ invoiceId, initialPdfUrl, onGenerated }: Props) {
  ...
  async function generate() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/invoice/${invoiceId}/generate-pdf`, { method: 'POST' })
      let data: { pdfUrl?: string; error?: string } | null = null
      try {
        data = await res.json()
      } catch {
  ...
  return (
    <div className="bg-white rounded-2xl border border-border p-5 mb-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {status === 'ready' && (
          <button onClick={generate} className="text-xs font-medium text-teal-500 hover:text-teal-700 transition">
            {t('regenerate')}
          </button>
        )}
      </div>
      ...
      {status === 'idle' && (
        <div className="text-center py-4">
          <p className="text-sm text-muted mb-4">{t('idleDescription')}</p>
          <button
            onClick={generate}
