...
export default function EmailQuoteSection({ jobId, pdfUrl, locale, initialEmailSentAt, initialEmailSentTo }: Props) {
  const t = useTranslations('emailQuoteSection')
  const tErrors = useTranslations('errors')

  const [status, setStatus] = useState<Status>(initialEmailSentAt ? 'sent' : 'idle')
  ...
  const [sentAt, setSentAt] = useState(initialEmailSentAt)
  ...
  async function draftEmail() {
    ...
    setDraftError(null)
    try {
      const res = await fetch(`/api/quote/${jobId}/draft-email`, { method: 'POST' })
  ...
  async function sendEmail() {
    ...
    setSendError(null)
    try {
      const res = await fetch(`/api/quote/${jobId}/send-email`, {
        method: 'POST',
...
function EmailReviewForm({
  ...
  t: ReturnType<typeof useTranslations<'emailQuoteSection'>>
